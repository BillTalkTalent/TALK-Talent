...
function CopyEventLinkButton({ eventId }: { eventId: string }) {
  ...
  async function handleCopy() {
    ...
    try {
      ...
      // nothing to recover to here, the link is still shareable manually.
    }
  }
  return (
    <button type="button" onClick={handleCopy} className="text-xs font-medium text-emerald-700 hover:underline">
    ...
    return (
      <div className="rounded-2xl p-5 space-y-3 bg-emerald-50 border border-emerald-100">
        ...
        {/* Peak-intent invite prompt — right after confirming, not buried
            ...
            no TALK account) so it works for an anonymous guest. */}
        <div className="pt-2 border-t border-emerald-100">
          <p className="text-xs font-semibold text-emerald-900 mt-2.5">Know someone else who should come?</p>
          <div className="flex items-center gap-3 mt-1">
            <CopyEventLinkButton eventId={eventId} />
            <a
              href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(typeof window !== "undefined" ? `${window.location.origin}/events/${eventId}` : "")}`}
              target="_blank"
              rel="noopener noreferrer"
