...
export default async function ChapterPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const canPost = isMember || isLead || currentProfile?.role === 'admin'
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl bg-white border border-zinc-100 shadow-sm p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="size-16 rounded-2xl flex items-center justify-center text-4xl bg-zinc-50 border border-zinc-100">
              {chapter.icon}
            </div>
            <div>
              ...
              <div className="flex items-center gap-3 mt-2 text-xs text-zinc-400">
                <span className="flex items-center gap-1"><Users className="size-3" />{members.length} member{members.length !== 1 ? 's' : ''}</span>
                {leads.length > 0 && (
                  <span className="flex items-center gap-1"><Star className="size-3 text-amber-400" />{leads.length} lead{leads.length !== 1 ? 's' : ''}</span>
                )}
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2 text-sm">
            {isMember ? (
              ...
              <Link href="/chapters" className="text-xs font-semibold text-zinc-500 bg-zinc-50 border border-zinc-200 px-3 py-1.5 rounded-xl hover:border-zinc-300 transition-colors">
                Join this chapter
              </Link>
            )}
          </div>
        ...
        {leads.length > 0 && (
          <div className="mt-5 pt-5 border-t border-zinc-100">
            <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-3">Chapter Lead{leads.length > 1 ? 's' : ''}</p>
            <div className="flex flex-wrap gap-3">
              {leads.map(lead => (
                <Link key={lead.id} href={`/members/${lead.id}`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-amber-100 bg-amber-50/50 hover:bg-amber-50 transition-colors group">
                  <Avatar className="size-9 ring-2 ring-amber-200">
                    {lead.avatar_url && <AvatarImage src={lead.avatar_url} alt={lead.full_name ?? ''} />}
                    <AvatarFallback className="text-xs font-bold bg-amber-100 text-amber-700">{getInitials(lead.full_name)}</AvatarFallback>
                  </Avatar>
                  <div>
                    <div className="flex items-center gap-1.5">
                      <p className="text-sm font-semibold text-zinc-900 group-hover:text-amber-700 transition-colors">{lead.full_name}</p>
      ...
      {/* Discussion board + member roster side by side */}
      <div className="grid gap-5 lg:grid-cols-3">
        {/* Discussion board — takes 2/3 */}
        ...
        <div className="rounded-2xl bg-white border border-zinc-100 shadow-sm p-5 space-y-3 h-fit">
            ...
            {members.slice(0, 20).map(m => (
              <Link key={m.id} href={`/members/${m.id}`} title={m.full_name ?? undefined}>
                <Avatar className="size-9 hover:ring-2 hover:ring-[#00d4aa] transition-all">
                  ...
                  <AvatarFallback className="text-xs font-bold" style={{ background: 'linear-gradient(135deg, #00b894, #00d4aa)', color: 'white' }}>
          ...
          {members.length > 20 && (
            <p className="text-xs text-zinc-400">+{members.length - 20} more members</p>
          )}
          {members.length === 0 && (
