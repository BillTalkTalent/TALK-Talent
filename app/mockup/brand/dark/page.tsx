...
const topics = [
  { title: 'How are you structuring sourcing teams in 2025?', replies: 24, views: 312, heat: '🔥🔥' },
  { title: 'AI screening tools: what actually works?', replies: 18, views: 267, heat: '🔥🔥' },
  { title: 'Comp benchmarking — which data sources do you trust?', replies: 15, views: 198, heat: '🔥' },
  { title: 'Navigating hiring freezes without losing pipeline', replies: 11, views: 145, heat: '🔥' },
  { title: 'Building employer brand on a shoestring budget', replies: 9, views: 121, heat: '🔥' },
]
...
export default function DarkMockup() {
  return (
    <div style={{ fontFamily: 'system-ui,-apple-system,sans-serif', background: C.bg, minHeight: '100vh', color: C.text }}>
      ...
      <div style={{ position: 'fixed', bottom: 20, left: '50%', transform: 'translateX(-50%)', zIndex: 100, display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)', padding: '8px 16px', borderRadius: 999, border: '1px solid #333', fontSize: 12, fontWeight: 700 }}>
        ...
        <span style={{ color: 'white', background: C.coral, padding: '3px 10px', borderRadius: 999 }}>Midnight Dark</span>
        <Link href="/mockup/brand/slate" style={{ color: '#666', padding: '3px 10px', borderRadius: 999, textDecoration: 'none' }}>Slate Pro</Link>
        <Link href="/mockup/brand/warm" style={{ color: '#666', padding: '3px 10px', borderRadius: 999, textDecoration: 'none' }}>Warm Espresso</Link>
        <Link href="/mockup/brand/plum" style={{ color: '#666', padding: '3px 10px', borderRadius: 999, textDecoration: 'none' }}>Deep Plum</Link>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
        ...
        {/* Welcome bar */}
        <div style={{ background: `linear-gradient(135deg, ${C.coral}22, ${C.purple}11)`, border: `1px solid ${C.coral}33`, borderRadius: 16, padding: '20px 24px', marginBottom: 28, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
          {[
            { label: 'Total Members', val: '473', delta: '+12 this month', color: C.coral },
            { label: 'Active Discussions', val: '953', delta: '+38 this week', color: C.purple },
            { label: 'Upcoming Events', val: '12', delta: 'Next: Jun 12', color: '#3B82F6' },
            { label: 'Open Jobs', val: '34', delta: '8 posted this week', color: '#10B981' },
          ].map(s => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 340px', gap: 20 }}>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '8px 0' }}>
              {members.map((m) => (
                <div key={m.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 20px' }}>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '8px 0' }}>
              {topics.map((t, i) => (
                <div key={t.title} style={{ padding: '12px 20px', borderBottom: i < topics.length - 1 ? `1px solid ${C.border}` : 'none', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  <div style={{ width: 22, height: 22, borderRadius: 6, background: i < 2 ? C.coral : C.surfaceAlt, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: i < 2 ? 'white' : C.textMuted, flexShrink: 0 }}>{i + 1}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: C.text, lineHeight: 1.4 }}>{t.title}</div>
                    <div style={{ fontSize: 11, color: C.textMuted, marginTop: 4 }}>{t.heat} {t.replies} replies · {t.views} views</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '8px 12px' }}>
                {events.map((e) => (
                  <div key={e.title} style={{ padding: '10px 8px', borderBottom: `1px solid ${C.border}` }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                      <div style={{ width: 36, height: 36, borderRadius: 8, background: e.badgeColor + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            {/* CTA card */}
            <div style={{ background: `linear-gradient(135deg, ${C.coral}, ${C.coralLight})`, borderRadius: 16, padding: 20 }}>
              <div style={{ fontSize: 15, fontWeight: 800, color: 'white', marginBottom: 6 }}>Refer a Colleague</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.8)', lineHeight: 1.5, marginBottom: 14 }}>Know a great TA leader? Invite them to the community.</div>
              <button style={{ width: '100%', padding: '9px 0', borderRadius: 10, background: 'white', color: C.coral, fontWeight: 800, fontSize: 13, border: 'none', cursor: 'pointer' }}>Send Invite</button>
            </div>
          </div>
