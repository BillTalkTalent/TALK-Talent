...
export default async function AdminJobsPage() {
  ...
  const closed = allJobs.filter(j => j.status !== 'active')
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {active.map(job => {
              ...
              return (
                <tr key={job.id} className="border-b border-zinc-100 hover:bg-zinc-50/50 transition-colors">
                  ...
                  <td className="py-3 pr-4 text-xs text-zinc-500">{JOB_TYPE_LABELS[job.job_type] ?? job.job_type}</td>
                  <td className="py-3 pr-4 text-xs text-zinc-500">{poster?.full_name ?? poster?.email ?? '—'}</td>
                  <td className="py-3 pr-4 text-xs text-zinc-400">{formatDistanceToNow(new Date(job.created_at), { addSuffix: true })}</td>
                  <td className="py-3 pr-4">
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-green-50 text-green-700 border border-green-100">Active</span>
                  </td>
                  <td className="py-3 pr-5">
                    <div className="flex items-center gap-2 justify-end">
                      <form action={toggleFeatured.bind(null, job.id, job.is_featured ?? false)}>
                        <button type="submit" title={job.is_featured ? 'Remove featured' : 'Mark featured'}
                      ...
                      <form action={toggleStatus.bind(null, job.id, job.status)}>
                        <button type="submit" title="Close listing" className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100 transition-colors">
                          <EyeOff className="size-3.5" />
                        </button>
                      </form>
                      <form action={deleteJob.bind(null, job.id)}>
                        <button type="submit" title="Delete job" className="p-1.5 rounded-lg text-zinc-300 hover:text-red-500 hover:bg-red-50 transition-colors">
            ...
            {closed.length > 0 && active.length > 0 && (
              <tr>
                <td colSpan={6} className="py-2 pl-5 text-xs font-bold text-zinc-400 uppercase tracking-wide bg-zinc-50/30 border-b border-zinc-100">
                  Closed listings
                </td>
              </tr>
            )}
            {closed.map(job => {
              ...
              const poster = (job as any).profiles as { full_name: string | null; email: string | null } | null
              return (
                <tr key={job.id} className="border-b border-zinc-100 hover:bg-zinc-50/50 transition-colors opacity-60">
                  <td className="py-3 pr-4 pl-5">
                    <div>
                      <Link href={`/jobs/${job.id}`} className="font-semibold text-sm text-zinc-900 hover:text-indigo-600 transition-colors">
                        {job.title}
                  ...
                  <td className="py-3 pr-4 text-xs text-zinc-400">{formatDistanceToNow(new Date(job.created_at), { addSuffix: true })}</td>
                  <td className="py-3 pr-4">
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-500">Closed</span>
                  </td>
                  <td className="py-3 pr-5">
                    <div className="flex items-center gap-2 justify-end">
                      <form action={toggleStatus.bind(null, job.id, job.status)}>
                        <button type="submit" title="Reopen listing" className="p-1.5 rounded-lg text-zinc-300 hover:text-green-600 hover:bg-green-50 transition-colors">
